    ...
    const fetchSpecialData = async () => {
      try {
        ...
        //   alert('Please login first');
          ...
          const response = await axios.get('http://localhost:8000/hotel/get-all-hotels', {
            headers,
          });

          // Assuming the response.data is an array of special data
                ...
                return (
                  <div key={index} className='w-full md:w-[250px] mx-1 my-1 md:my-0 shadow-md border-[1px] rounded-md'>
                    <div className='w-full md:w-[210px] h-[130px]'>
                      <img src={data.img || ViewImg} alt='viewImages' className='w-full h-full rounded-tl-md rounded-tr-md' />
                    </div>
                    <div className='w-full md:w-[210px]  px-1 pt-2 pb-[2px]'>
                      <h3 className='leading-7  text-[25px]'>{data.Header || 'Special Header'}</h3>
                      <p className='text-[14px] leading-5'>{data.para || 'Special Paragraph'}</p>
                    </div>
                  </div>
                );
              })
